    ...
    for key, hexKeyCode in virtual_key_codes:
        ...
        if key in extendedKeys:
            ...
            if ctypes.windll.user32.GetKeyState(0x90):
                ...
                ii_ = Input_I()
                ii_.ki = KeyBdInput(0, 0xE0, KEYEVENTF_SCANCODE, 0, ctypes.pointer(extra))
...
def keyHold(*keys):
    ...
    This function simulates pressing and holding one or more specified keys. The keys parameter should be a string, not a list.
    ...
    for key, hexKeyCode in virtual_key_codes:
        ...
        if key in extendedKeys:
            ...
            if ctypes.windll.user32.GetKeyState(0x90):
                ...
                extra = ctypes.c_ulong(0)
                ii_ = Input_I()
                ii_.ki = KeyBdInput(0, 0xE0, KEYEVENTF_SCANCODE | KEYEVENTF_KEYUP, 0, ctypes.pointer(extra))
                x = Input(ctypes.c_ulong(1), ii_)
                ctypes.windll.user32.SendInput(1, ctypes.pointer(x), ctypes.sizeof(x))
...
def keyPress(keys, interval=default_interval, presses=1, simultaneously=False):
    ...
    This function simulates pressing and releasing one or more specified keys. The keys parameter can be a single key or a list of keys. The keys can be pressed either simultaneously or sequentially based on the `simultaneously` parameter. Additionally, the function allows specifying the number of times to press the keys and the interval between presses.

    Parameters:
    keys : str or list of str
        The key or list of keys to press. The key names should correspond to the key mappings.
    interval : float, optional
        The interval between key presses in seconds (default is the module's default_interval).
    presses : int, optional
        The number of times to press the keys (default is 1).
    simultaneously : bool, optional
        Whether to press all keys at once (default is False). If False, keys are pressed sequentially.
    ...
    if simultaneously:
        for _ in range(presses):
            ...
            for key in keys:
                ...
                extra = ctypes.c_ulong(0)
                ii_ = Input_I()
                ii_.ki = KeyBdInput(0, hexKeyCode, keybdFlags | KEYEVENTF_KEYUP, 0, ctypes.pointer(extra))
...
def hotKey(*keys, interval=default_interval):
    ...
    This function simulates pressing and holding down a combination of keys simultaneously, followed by releasing them. The keys parameter can accept multiple key arguments to form the hotkey combination.

    Parameters:
    *keys : str
        One or more keys to press as part of the hotkey combination. The key names should correspond to the key mappings.
    interval : float, optional
        The interval between each key press and release in seconds (default is the module's default_interval).
...
def write(text: str, speed=0.0, interval=0.03):
    ...
    This function simulates typing out a given text string character by character. Optionally, a speed (in seconds) between key presses can be specified to simulate a more natural typing speed. If a character is not recognized, it will be copied and pasted from the clipboard.
    ...
    interval : float, optional
        The interval (in seconds) between key press and release for each character (default is 0.03).
...
def keyDetect(keys):
    ...
    This function checks the state of one or more specified keys to determine if they are currently pressed. The keys parameter can be a single key (as a string) or a list of keys. The function returns True if all specified keys are pressed, and False if any key is not pressed or not recognized.
...
def mouseClick(button='left', interval=default_interval, presses=1):
    ...
    This function simulates mouse click events for a specified mouse button. The button parameter determines which mouse button to click, and the presses parameter specifies the number of times to click the button.
...
def mouseDown(button='left'):
    ...
    This function simulates pressing down a specified mouse button. The button parameter determines which mouse button to press.
...
def mouseUp(button='left'):
    ...
    This function simulates releasing a specified mouse button. The button parameter determines which mouse button to release.
...
def mouseHold(button='left'):
    ...
    This function simulates pressing and holding down a specified mouse button. The button parameter can be 'left', 'right', or 'middle'. This function should be used with a `with` statement to ensure that the mouse button is released after the block of code is executed.
...
def moveMouseTo(x=None, y=None, duration=0.0):
    ...
    This function moves the mouse cursor to a specified (x, y) position on the screen over a specified duration. If either x or y is not provided, the current mouse position for that axis will be used.
    ...
    duration : float, optional
        The duration over which the mouse cursor should move to the target position, in seconds. Default is 0.0, which moves the cursor instantly.
...
def moveMouse(xOffset=0, yOffset=0, duration=0.0):
    ...
    This function moves the mouse cursor from its current position by a specified x and y offset over a given duration.
    ...
    duration : float, optional
        The duration over which the mouse cursor should move to the new position, in seconds. Default is 0.0, which moves the cursor instantly.
...
def scrollMouse(clicks):
    ...
    This function simulates scrolling the mouse wheel vertically. The `clicks` parameter determines the amount and direction of the scroll. Positive values scroll up, while negative values scroll down. For a visible result, it is recommended to use a value of 100 or more, as smaller numbers may not produce a significant scrolling effect.
...
def screenshot(filename=None, region=None):
    ...
    This function captures a screenshot of the entire screen or a specified region. If a region is specified, it captures the region defined by the tuple (top-left x, top-left y, width, height). The screenshot can be saved to a file if a filename is provided.

    Parameters:
    filename : str, optional
        The file path to save the screenshot. If not specified, the screenshot is not saved. You can put any file type such as .png, .jpg or .pdf.
    region : tuple, optional
        A tuple specifying the region to capture (top-left x, top-left y, width, height). If not specified, captures the entire screen.
...
def locateImage(needleImage, haystackImage=None, grayscale=False,
    ...
    This function searches for a smaller image (needleImage) within a larger image (haystackImage) or the entire screen. If haystackImage is not provided, the function captures the entire screen for searching. The search can be performed in grayscale for improved performance, and a specific region can be defined for the search. The threshold parameter sets the accuracy required for a match.
    ...
    region : tuple, optional
        A tuple specifying the region to search within (top-left x, top-left y, width, height). If not specified, the entire image or screen is used.
