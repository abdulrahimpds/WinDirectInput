...
def keyDown(*keys):
    ...
    for key, hexKeyCode in virtual_key_codes:
        keybdFlags = 0x0008  # KEYEVENTF_SCANCODE
        ...
        if key in ['up', 'down', 'left', 'right']:
            keybdFlags |= 0x0001  # KEYEVENTF_EXTENDEDKEY
            ...
            if ctypes.windll.user32.GetKeyState(0x90):
                ...
                ii_ = Input_I()
                ii_.ki = KeyBdInput(0, 0xE0, 0x0008, 0, ctypes.pointer(extra))
...
def keyUp(*keys):
    ...
    for key, hexKeyCode in virtual_key_codes:
        keybdFlags = 0x0008 | 0x0002  # KEYEVENTF_SCANCODE | KEYEVENTF_KEYUP
        ...
        if key in ['up', 'down', 'left', 'right']:
            ...
            if ctypes.windll.user32.GetKeyState(0x90):
                ...
                extra = ctypes.c_ulong(0)
                ii_ = Input_I()
                ii_.ki = KeyBdInput(0, 0xE0, 0x0008 | 0x0002, 0, ctypes.pointer(extra))
                x = Input(ctypes.c_ulong(1), ii_)
                ctypes.windll.user32.SendInput(1, ctypes.pointer(x), ctypes.sizeof(x))
    ...
    if simultaneously:
        for _ in range(presses):
            ...
            for key in keys:
                ...
                extra = ctypes.c_ulong(0)
                ii_ = Input_I()
                ii_.ki = KeyBdInput(0, hexKeyCode, keybdFlags | 0x0002, 0, ctypes.pointer(extra))
